...
const removeUndefinedProperties = (obj) =>
    Object.fromEntries(Object.entries(obj).filter((entry) => entry[1] !== undefined));
...
var body = { stuff: 'is' };
var config = {};
var callback = (operation, done) => (err, data, meta) => {
    if (err) {
        return done(err);
    }
...
var resolve = (operation, done) => (result) => {
    try {
        ...
        expect(result.data.args.params).to.eql(removeUndefinedProperties(params));
